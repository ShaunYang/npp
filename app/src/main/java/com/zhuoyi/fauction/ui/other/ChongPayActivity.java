 ...
 * 
 */
public class ChongPayActivity extends BaseActivity {

	@Bind(R.id.your_money)
	EditText yourMoney;
	@Bind(R.id.alipy_pay) ImageView alipyPay;

	int payMethod=1;//1支付宝 2网银
...
//		proId=intent.getIntExtra("productId",0);
	...
	private void chongPayPost() {
		String timestamp= DateUtil.getStringDate();

		String sign= Util.createSign(getApplicationContext(),timestamp,"Wallet","recharge");


		OkHttpUtils.post()
				.url(Constant.WALLET_RECHARGE)
				...
				.addParams("price",yourMoney.getText().toString().trim())
				.addParams("pay_type",1+"")
				.addParams("price_key", MD5Util.getMD5String(MD5Util.getMD5String(yourMoney.getText().toString().trim()) + 1))
				.build()
				.execute(new StringCallback() {
					...
					public void onResponse(String response) {
						...
						if (ret.getCode() == 0) {
							...
							intent.setClass(ChongPayActivity.this, WalletPayActivity.class);
							intent.putExtra("oid", oid);
							intent.putExtra("total",yourMoney.getText().toString().trim());
							startActivity(intent);*/

						} else if (ret.getCode() == -1) {
							ToastUtil.showLongToast(ChongPayActivity.this, "登录超时");
						} else if (ret.getCode() == -2) {

							ToastUtil.showLongToast(ChongPayActivity.this, "充值金额不对");
						}
...
//							startActivity(intent);
							...
							Runnable payRunnable = new Runnable() {
								...
								public void run() {
									// 构造PayTask 对象
									PayTask alipay = new PayTask(ChongPayActivity.this);
									// 调用支付接口，获取支付结果
							...
							ToastUtil.showLongToast(ChongPayActivity.this,"没有找到订单数据");
						}
					}
				});
	}
			...
			switch (msg.what) {
				case SDK_PAY_FLAG: {
					...
					if (TextUtils.equals(resultStatus, "9000")) {
						Toast.makeText(ChongPayActivity.this, "充值支付成功", Toast.LENGTH_SHORT).show();
						flag=true;
						...
						if (TextUtils.equals(resultStatus, "8000")) {
							Toast.makeText(ChongPayActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
							flag=false;

						} else {
							// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
							Toast.makeText(ChongPayActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
							flag=false;
...
//					intent.putExtra("oid", oid);
//					startActivity(intent);
					Intent intent=new Intent(ChongPayActivity.this, MineWalletActivity.class);
					startActivity(intent);
					finish();
					break;
				}
...
//							startActivity(intent);
							 ...
							 * 步骤2：通过银联工具类启动支付插件
							 ************************************************/
							UPPayAssistEx.startPay(ChongPayActivity.this, null, null, tn, mMode);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		builder.create().show();*/
		Toast.makeText(ChongPayActivity.this,msg , Toast.LENGTH_SHORT).show();
		Intent intent=new Intent(ChongPayActivity.this,MineWalletActivity.class);
